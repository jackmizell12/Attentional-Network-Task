...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\jackm\\Desktop\\PsychoPy\\ANT\\ANT.py',
...
MainTrialInstructClock = core.Clock()
...
_key_resp_5_allKeys = []
# keep track of which components have finished
practice_instrComponents = [pract_instr, key_resp_5]
...
for thisPractice in practice:
    ...
    while continueRoutine:
        ...
        # *pract_resp* updates
        waitOnFlip = False
        if pract_resp.status == NOT_STARTED and tThisFlip >= stimStart-frameTolerance:
            # keep track of start time/frame for later
...
_key_resp_6_allKeys = []
# keep track of which components have finished
trial_instrComponents = [practice_res, key_resp_6]
...
_key_resp_7_allKeys = []
# keep track of which components have finished
MainTrialInstructComponents = [text_5, key_resp_7]
for thisComponent in MainTrialInstructComponents:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
MainTrialInstructClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "MainTrialInstruct"-------
while continueRoutine:
    # get current time
    t = MainTrialInstructClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=MainTrialInstructClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    ...
    for thisTrial in trials:
        ...
        while continueRoutine:
            ...
            if post_fixation.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > post_fixation.tStartRefresh + frameTolerance:
                    # keep track of stop time/frame for later
